...
async fn complete_internal(
    ...
    if !response.status().is_success() {
        ...
        if status == 429 {
            tracing::warn!(status, retry_after, model = %config.model, body_preview = %&body[..body.len().min(200)], "OpenRouter LLM rate limited");
            return Err(ProviderError::RateLimited {
                retry_after_secs: retry_after,
            });
        }

        tracing::error!(status, model = %config.model, body_preview = %&body[..body.len().min(500)], "OpenRouter LLM API error");
    ...
    let chat_response: ChatResponse = serde_json::from_str(&body)
        .map_err(|e| {
            tracing::error!(error = %e, model = %config.model, body_preview = %&body[..body.len().min(500)], "OpenRouter LLM parse error");
...
async fn complete_streaming_internal(
    ...
    if !response.status().is_success() {
        ...
        if status == 429 {
            tracing::warn!(status, retry_after, model = %config.model, body_preview = %&body[..body.len().min(200)], "OpenRouter streaming LLM rate limited");
            return Err(ProviderError::RateLimited {
                retry_after_secs: retry_after,
            });
        }

        tracing::error!(status, model = %config.model, body_preview = %&body[..body.len().min(500)], "OpenRouter streaming LLM API error");
        return Err(ProviderError::Api {
            status,
            message: body,
        });
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        while let Some(line_end) = buffer.find('\n') {
            ...
            if let Some(json_str) = line.strip_prefix("data: ") {
                ...
                Err(e) => {
                    tracing::debug!(error = %e, chunk_preview = %&json_str[..json_str.len().min(200)], "OpenRouter stream chunk parse error");
                }
                Ok(response) => {
